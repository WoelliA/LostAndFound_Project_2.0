...
(function (Helpers) {
    var InstanceLoader = (function () {
        ...
        InstanceLoader.prototype.getInstance = function (name) {
            ...
            var ref = this.getReference(name);

            var instance = Object.create(ref.prototype);
    ...
    var InitInstanceLoader = (function (_super) {
        __extends(InitInstanceLoader, _super);
        function InitInstanceLoader(context, parameters) {
            _super.call(this, context, null);
            this.parameters = parameters;
        }
        InitInstanceLoader.prototype.getInstance = function (name) {
            var args = [];
            for (var _i = 0; _i < (arguments.length - 1); _i++) {
                args[_i] = arguments[_i + 1];
            }
            args = this.addParameters(args);
            var ref = this.getReference(name);
            ref.init(args);
        };
        ...
        return InitInstanceLoader;
    })(InstanceLoader);
    Helpers.InitInstanceLoader = InitInstanceLoader;
})(Helpers || (Helpers = {}));
...
(function (Presenting) {
    var Loader = (function () {
        ...
        Loader.prototype.load = function (target, loadingResult, callback) {
            var _this = this;
            target = this.resolveTarget(target);
            if (!target) {
                this.finish(null, loadingResult, callback);
            }

            if (this.cache[target]) {
                this.finish(this.cache[target], loadingResult, callback);
            } else {
                ...
                $.ajax(filename).done(function (result) {
                    ...
                    if (target !== "404")
                        _this.load("404", loadingResult, callback);
                });
            }
        };

        Loader.prototype.onComplete = function (result) {
        };
        ...
        Loader.prototype.resolveTarget = function (target) {
            if (this.settings.mappings != null) {
        ...
        Loader.prototype.finish = function (result, loadingResult, callback) {
            loadingResult.results[this.settings.fileNameEnding] = result;
            if (this.onLoadingFinished) {
                this.onLoadingFinished(result);
            }
            callback();
        };

        Loader.prototype.targetToClassName = function (target) {
            var first = target.charAt(0).toUpperCase();
            var objectName = first + target.substr(1) + this.settings.fileNameEnding;
            return objectName;
        };

        Loader.prototype.isSubLoader = function () {
            return true;
        };
        return Loader;
    })();
    ...
    var HtmlLoader = (function (_super) {
        ...
        function HtmlLoader(settings) {
            _super.call(this, settings);
        }
        HtmlLoader.getDefaultSettings = function (root) {
            var viewLoaderSettings = {
                rootDir: root,
            ...
            return viewLoaderSettings;
        };
        return HtmlLoader;
    })(Loader);
    Presenting.HtmlLoader = HtmlLoader;
    ...
    var ViewLoader = (function (_super) {
        ...
        ViewLoader.getDefaultSettings = function (root) {
            var loaderSettings = {
                rootDir: root,
                targetDir: "src/views",
                extension: ".js",
    ...
    var ControllerLoader = (function (_super) {
        __extends(ControllerLoader, _super);
        function ControllerLoader(settings, instanceLoader) {
            _super.call(this, settings);
        ...
        ControllerLoader.getDefaultSettings = function (root) {
            var loaderSettings = {
                rootDir: root,
                targetDir: "src/controllers",
                extension: ".js",
                fileNameEnding: "Controller",
                mappings: []
            };
            return loaderSettings;
        };

        ControllerLoader.prototype.isSubLoader = function () {
            return false;
        };
        return ControllerLoader;
    })(Loader);
    Presenting.ControllerLoader = ControllerLoader;
})(Presenting || (Presenting = {}));
...
(function (Presenting) {
    ...
    var PresentationRequest = (function () {
        ...
        PresentationRequest.prototype.processLoaders = function (donecallback) {
            var _this = this;
            ...
            for (var i in this.loaders) {
                ...
                loader.load(this.target, this.result, function () {
                    loadersFinished++;
                    if (loadersFinished == _this.loaders.length) {
                        donecallback();
                    }
                });
            }
        ...
        PresentationRequest.prototype.loadHtml = function (doneCallback) {
            this.htmlLoader.load(this.target, this.result, function () {
                return doneCallback();
            });
        };

        PresentationRequest.prototype.tryFinish = function () {
            if (this.loadersDone && this.viewLoaderDone) {
                for (var i in this.loaders) {
                    var loader = this.loaders[i];
                    loader.onComplete(this.result);
                }
            }
        };
    ...
    var Presenter = (function () {
        function Presenter(settings) {
            ...
            this.contentListeners = settings.contentListeners;

            this.htmlLoader = settings.htmlLoader;
        }
        Presenter.prototype.getFrame = function () {
            return this.frame;
        };

        Presenter.prototype.getSettings = function () {
            return this.settings;
        };

        Presenter.prototype.show = function (target, parameter, callback) {
            var _this = this;
            ...
            request.result.frame = this.frame;
    ...
    Presenting.Presenter = Presenter;
        ...
        DataContentListener.prototype.onContentLoaded = function (presenter, content, parameter) {
            ...
            if (dataDefers) {
                for (var i = 0; i < dataDefers.length; i++) {
                    ...
                    var s = $.extend(false, {}, presenter.getSettings());

                    var subloaders = [];
                    for (var i in s.loaders) {
                        var loader = s.loaders[i];
                        if (loader.isSubLoader()) {
                            subloaders.push(loader);
                        }
                    }
                    s.loaders = subloaders;

                    s.frame = el;
                    var newPresenter = new Presenter(s);
                    newPresenter.show(deferTarget, parameter);
                }
            }
        };
        return DataContentListener;
    })();
...
(function (Presenting) {
    ...
    Presenting.LoadingResult = LoadingResult;
})(Presenting || (Presenting = {}));
